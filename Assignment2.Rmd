...
```{r}
write.csv(dftrain,"dftrain.csv",row.names=FALSE)
write.csv(dftest,"dftest.csv",row.names=FALSE)
```

```{r}
dftrain <- read.csv("dftrain.csv",header = TRUE)
dftest <- read.csv("dftest.csv",header = TRUE)
```
...
# Forecasting capability of the final model in the work sample (lab)
...
tt <- table(target.est,dftrain$target);tt #confusion matrix
...
# Calculate the pseudo coefficient of determination of the final model and the Naglekerke coefficient. Do a goodness of fit test to the final model. Calculate the goodness of fit using the statistic proposed by Hosmer-Lemershow.
...
library(cvAUC)
AUC(predict(m12,type="response"),dftrain$target)

# Confusion Matrix in ModelMetric library
library(ModelMetrics)
ModelMetrics::confusionMatrix(dftrain$pres, prob.vot)
...
summary(dfwork.fin)
100*(1-dfwork.fin$dev/dfwork.fin$null.dev) # pR2 McFadden

# Estadístic de Hosmer-Lemershow
# seque<-c(seq(0,1,by=0.1));seque  # Original proposal
seque<-quantile(fitted(dfwork.fin),probs=seq(0,1,by=0.1)) #get the quatiles
fitgrup<-cut(fitted(dfwork.fin),breaks=seque); fitgrup #we create groups in term of the breaks -> each prediction belongs to 1 break
cfitgrup<-table(dfwork$pres,fitgrup);cfitgrup # table that tells me how many real 1 and 0 there are in each group
# we see that the number of 1s increases in the groups
# there were 42 badly predicted in the first group

cmodel1<-tapply(fitted(dfwork.fin),fitgrup,sum);cmodel1 #fitted: estimated probabilities, we sum the probabilities of the fitted values
#the probabilities are the expected number of positive predicted cases inside each group
cmodel0<-tapply(1-fitted(dfwork.fin),fitgrup,sum);cmodel0 # the expected number of negative cases inside each group

cmodelgrup <- rbind(cmodel0,cmodel1);cmodelgrup #predicted data
# if model is perfect, we expect the cmodelgrup and cfitgrup to match

XHL<-sum(((cfitgrup-cmodelgrup)^2)/cmodelgrup);XHL #we check the square difference and dividing by the real value
# this gives me a value that follow a chisquare distribution

dfXHL<-8 #g-2 df; g: number of breaks that we have done, 10 breaks -2 = 8 df
1-pchisq(XHL,dfXHL)
# the higher this number, the distance is bigger, it tells me 0.001234, H0: model fits data, distance between both matrices is low
# pvalue = 0.001234 --> reject H0 --> hoslem: model could not fit the data
# esto es lo mismo que hoslem.test() --> aplicar esto directamente

# Determine the predictive power by analyzing the ROC curve.
library("ROCR")
dadesroc<-prediction(predict(dfwork.fin,type="response"),dfwork$pres)
par(mfrow=c(1,2))
...
library(cvAUC)
AUC(predict(dfwork.fin,type="response"),dfwork$pres)
...
dfwork.fin <- m12
prob.vot <- predict(dfwork.fin, newdata=dftest, type="response")
pres.est <- ifelse(prob.vot<0.5,0,1)
table(pres.est,dftest$pres)
sum(diag(table(pres.est,dftest$pres)))/dim(dftest)[1]

# Model na?ve dummy model
m0<-glm(pres ~ 1, family=binomial, data=dftest)
summary(dfwork$pres) #mean = 0.7 --> more people on the sample choose vote, so the model fitted with this will predict 1 on the whole test set
prob.vot0 <- m0$fit
pres.est0 <- ifelse(prob.vot0<0.5,0,1)
table(pres.est0,dftest$pres)
table(pres.est0,dftest$pres)[1,2]/dim(dftest)[1]
# 0.67, nuestro modelo que hemos hecho no ha sido MUCHO mejor que el dummy model, deberíamos mejorar nuestro modelo
