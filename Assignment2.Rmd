...
# res.cat <- catdes(df, which(names(df) == 'nb_miss'))
# 
# res.cat$quanti.var
# res.cat$test.chi2
# res.cat$quanti
...
# Work on character to avoid factor level issues
df$major_discipline <- as.character(df$major_discipline)

df$major_discipline <- case_when(
  # 1) No major because level of education does not have a major
  ...
  # 4) Everything else (including real missing from Graduate/Master)
  TRUE ~ NA_character_
)
