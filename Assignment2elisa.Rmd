...
- target = 1 means a candidate is looking for a new job (likely to come to our company after the training), so the candidate will stay in our company after training.
- target = 0 means a candidate is not looking for a new job (likely to not come to our company after the training), so the candidate will leave our company after training to stay in their current job.
...
- False negative: model predicts 'leaves' (0) and actually 'will stay' (1). In this case, you skip a good candidate, but in this case, the cost is real, but lower than the FP one.
Therefore, FP are more expensive than FN. 
...
tt0 <- table(target.est0,dftrain$target); tt0
