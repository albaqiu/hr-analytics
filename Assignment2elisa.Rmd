...
Using the profiling of the missing values per variable, we identified three different behaviors:
  ...
  - *Structural and Informative Missingness*: We confirmed that `major_discipline` is missing mainly for candidates with "High School" education, meaning the variable is just not applicable (it is not a real missing). Similarly, `company_size` and `company_type` are missing when the candidate has no relevant experience or any previous employment.

We will recode these to "No Major" and "Undefined" respectively, as this "missing" info is actually describing the candidate's situation (e.g., unemployment).
...
#### enrolled_university
The variable `enrolled_university` contains 386 missing values and 3 levels: 'Full time course', 'no_enrollment' and 'Part time course'. The purpose of this variable is to indicate whether the person is currently studying or not. It is not easy to distinguish patterns for the missingness, because there is no clear path for a persons career (although unlikely, for example, maybe they have a Masters in a different degree and they are studying something else). For this reason, we decided to impute the missing values.
...
```
```{r}
# table(df$major_discipline[is.na(df$education_level)])
# table(df$education_level)
```
...
res.cat$category
```
With this, we can analyze which variables are statistically associated with having more or fewer missing values. After the profiling, we confirm that:
...
- education_level: graduate, masters, PhD are strongly associated with 0 missing, while High School and Primary → associated with higher missingness
...
RESUM DEL QUE HE FET:
m1: todas las numericas. todas significativas
m4: polycity2. todas signif. 
m5: polycity3. todas signif.
m8: polyexperience2. todas signif.
m6. cambiamos polyexperience por factor y da 20 mejor de aic como numerica. todo signif.
m5b: probamos con training as a factor y step lo quita, pero Anova lo pone signif. y el aic es 10 unidades mas bajo con numerica.

RESULTADO. polycity3 + polyexp2 + traning_hours
...
```
In the model with all numeric variables, all of them are significative.
```{r}
...
As cubic effect of `city_development_index` has lower AIC, we choose this transformation to be applied.

As we apply a quadratic effect to `experience`, we see that AIC is lower, we compare this to using the factor version of this variable.
```{r}
...
```
```{r}
# Model with poly(city_development_index, 3) + f.experience
...
Now we check the factor version of training.
```{r}
...
AIC(m0, m5, m5b)
```
We observe from Anova() that `f.training` is not significative. So, in conclusion, m5 is the final model after checking the numeric variables.
...
Finally, including the significant interactions: relevent_experience:last_new_job + poly(city_development_index,3):company_size, we fit the final model.
```{r}
...
AQUEST ERA EL MODEL QUE HAVIEM CONSIDERAT INICIALMENT
```{r}
mfinold <- glm(target ~ poly(city_development_index,3) + relevent_experience + 
...
llhat <- which(hatvalues(mfinal) > 3*length(coef(mfin))/nrow(dftrain)); llhat
common_outliers <- intersect(llres, llhat); common_outliers
llcoo <- Boxplot(cooks.distance(mfinal), id=list(n=10,labels=row.names(dftrain)))

lldist <- which(cooks.distance(mfinal)>0.005); lldist

llrem <- which( rownames(dftrain) %in% c("18066"));llrem # Returns row numbers in dfwork for row names 
...
llrem <- which( rownames(dftrain) %in% c("11187","14820","11205","15016"));llrem # Returns row numbers in dfwork for row names 
# we remove the ones that lose continuity
...
influencePlot(mfin5)
llrem <- which( rownames(dftrain) %in% c("7985"));llrem # Returns row numbers in dfwork for row names 
# we remove the ones that lose continuity
...
In the influent data assesment we removed 11 observations because they have larger Cook's distance.
...
1 = candidate is looking for a new job (likely to come to our company after the training)
0 = candidate is NOT looking for a new job (likely to not come to our company after the training, so, leave our company)
...
Predicted     0      1
        ...
        1   1751   2236
We have:
- 9713 TN: people predicted to leave and actually leaves.
- 1751 FP: people predicted to stay and actually leaves.
- 1593 FN: people predicted to leave that actually stays.
- 2236 TP: people predicted to stay that actually stays.

To interpret the model, we need to know which mistake is worse for the company:
- False positive: model predicts 'will stay' (1) and actually 'leaves'(0). In this case, Company invests training but candidate leaves, leading to unnecessary costs.
- False negative: model predicts 'leaves' (0) and actually 'will stay' (1). In this case, you skip or deprioritize a good candidate, but in this case, cost is real, but lower than FP.
So FP are more expensive than FN. 

The model correctly classifies a large number of candidates who will leave, as reflected by the high number of True Negatives (9713). However, given the dominance of class 0 in the data, this result is expected and does not necessarily indicate strong predictive ability. A model biased toward predicting the majority class can achieve high apparent performance by primarily predicting "will leave". In contrast, the model performs worse on the minority class. The number of True Positives (2236) is relatively low compared to the number of candidates who actually stay, indicating limited ability to identify stayers.

We obtain a F1 score of 0.57, which is lower than expected, but if we compare with the dummy model (0.40), it has improved significantly. Also, to improve this score, we fitted other models to see if adding variables would improve the prediction power, but even with the complete model (with all variables and interactions), the maximum F1 score obtained was 0.605. Due to the low difference in the F1 score, we prioritize a simpler model even if F1 is lower.

ROC curves and AUC are used as complementary tools to assess the model's overall discriminative ability independently of any classification threshold. However, due to class imbalance and asymmetric misclassification costs, ROC/AUC are not used as primary evaluation metrics. AUC = 0.79, this suggests that the model separates stayers from leavers reasonably well, though there is room for improvement.
...
# Confusion Matrix
tt <- table(target.est,dftrain$target);tt #compare estimation and actual target, gives confusion matrix

library(ModelMetrics) # we can also use a library
ModelMetrics::confusionMatrix(dftrain$target, prob) #this has threshold 0.5
...
# ROC curve and AUCto evaluate discrimination ability independently of threshold
library(cvAUC)
...
In the test sample, we expect similar results to those in the train sample to assess whether the model is overfitting. We observe that the metrics do not differ much, indicating that the model generalizes well to the test sample.
...
# Confusion Matrix
tttest <- table(target.test,dftest$target);tttest #confusion matrix
...
target.test0 <- ifelse(prob0test<0.5,0,1)
tt0test <- table(target.test0,dftest$target)
tt0test

table(target.test0,dftest$target)[1,2]/(dim(dftest)[1])
precision0test <- tt0test[1,2]/(tt0test[1,2] +tt0test[1,1]);precision0test
recall0test <- tt0test[1,2]/(tt0test[1,2] +0 );recall0test
f10test <-2*((precision0test*recall0test)/(precision0test+recall0test)); f10test
```
...
*1) `training_hours`*
First of all, training_hours has a coefficient of -0.001202, the negative sign suggests that candidates with more training hours are slightly more likely to leave.

*2) `relevent_experience`*
All else being equal (ceteris paribus), candidates without relevant experience have lower odds of staying compared to those with relevant experience, as indicated by the negative coefficient for relevent_experienceNo (β = −0.265). This implies that, relative to the reference group (relevent_experienceYes), the odds of staying for candidates without relevant experience are multiplied by exp(−0.265) = 0.77, so going from relevent_experienceYes to relevent_experienceNo leads to a reduction of approximately 23% in the odds of staying, except where this effect is modified by interactions with last_new_job.
...
ENCARA NO L'HE INTERPRETAT PQ M'ESTIC LIANT

*4) `education_level`*
Regarding education level, candidates with High School, Primary School, Master’s, or PhD education all have significantly lower odds of staying compared to the reference education category. For instance, candidates with only Primary School education have odds of staying multiplied by exp(-1.28) = 0.28, so going from education_levelGraduate to education_levelPrimary School leads to a reduction of 72% the odds of staying.
...
*5) `company_size`*
Regarding the company_size, candidates working in very large companies or in companies with undefined size show significantly higher odds of staying compared to the reference company size, with especially strong effects for the “Undefined” category. However, because company size interacts with the nonlinear city development index, these effects vary depending on the level of city development.
