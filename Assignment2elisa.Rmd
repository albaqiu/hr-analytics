...
If we want to find out the probability of a person who:
- lives in a city with `city_development_index` 
- gender = Female
```{r}
summary(mfin)
exp(coef(m1)[2])
mean(elecc92$pres)*(1-mean(elecc92$pres))*coef(m1)[2]
# Interpret the estimated coefficients in terms of logodds, odds, and probabilities.
# Logodds of voting increase by 0.0135 units by year
# Odds of voting are multiplied by 1.014 each year or increased by 1.4% every year (exp(beta2)-1)*100.
# Probability of voting increases by 0.0029 absolute unit each year of age
# Which would the probability of voting be for a citizen on the median of age in the sample (42 years)
median(elecc92$age)
eta <- coef(m1)[1] + coef(m1)[2]* median(elecc92$pres); eta
exp(eta) # 1.26 to 1
exp(eta)/(1+exp(eta)) #probability of them going to vote is 55%
```