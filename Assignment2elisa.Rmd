...
Using the profiling of the missing values per variable, we identified three different behaviors:
  ...
  - *Structural and Informative Missingness*: We confirmed that major_discipline is missing mainly for candidates with "High School" education, meaning the variable just doesn't apply (it is not a real missing). Similarly, company_size and company_type are missing when the candidate has no relevant experience or any previous employment.
...
tt <- table(target.est,dftrain$target)
...
MFINOLD (el inicial)
```{r}
prob <-mfinold$fit #gives a vector of n observation with probability of voting of each individual
target.est <- ifelse(prob<0.5,0,1) #apply threshold
...
PseudoR2(mfin, which='all') # library(DescTools)
hoslem.test(dftrain$target, fitted(mfinold), g=10) # ResourceSelection library

sum(residuals(mfinold, type='pearson')^2) # Pearson's X2
library(cvAUC)
AUC(predict(mfin,type="response"),dftrain$target)
```
...
tt <- table(target.test,dftest$target)
...
```{r}
probtest <- predict(mfinold, newdata=dftest, type="response")
target.test <- ifelse(probtest<0.4,0,1)
table(target.test,dftest$target)
tt <- table(target.test,dftest$target)
...
PseudoR2(mfinold, which='all') # library(DescTools)
hoslem.test(dftest$target, probtest)
```
