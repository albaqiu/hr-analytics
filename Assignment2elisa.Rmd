...
m4 <- glm(target ~ poly(city_development_index,2) + training_hours + experience, family="binomial", data = dftrain)
summary(m4)

vif(m4)
step(m4, k= log(nrow(dftrain)))
Anova(m4, test="LR")
anova(m1, m4, test="Chisq") # H0 rejected, model with transformation is better
AIC(m0, m1, m4)
```
...
m5 <- glm(target ~ poly(city_development_index,3) + training_hours + experience, family="binomial", data = dftrain)
summary(m5)
...
m8 <- glm(target ~ poly(experience,2) + poly(city_development_index,3) + training_hours, family="binomial", data = dftrain)
summary(m8)
...
m6 <- glm(target ~ f.experience + poly(city_development_index,3) + training_hours, family="binomial", data = dftrain)
summary(m6)
...
step deja todas las numericas + estos factores: enrolled_university + major_discipline + 
    company_size + last_new_job

```{r}
# Model with all factors
m11 <- glm(target ~ poly(city_development_index,3) + poly(experience,2) + training_hours + gender + relevent_experience + enrolled_university + education_level + major_discipline + company_size + company_type + last_new_job, family="binomial", data = dftrain)
...
m12 <- glm(target ~ poly(city_development_index, 3) + poly(experience, 2) + 
    training_hours + enrolled_university + major_discipline + 
...
comentari: he afegit les interaccions amb les altres numeriques
...
```{r}
mi1 <- glm(target ~ poly(city_development_index,3)*(enrolled_university + major_discipline + 
    company_size + last_new_job), family="binomial", data = dftrain)
...
```
```{r}
mi1 <- glm(target ~ poly(experience,2)*(enrolled_university + major_discipline + 
    company_size + last_new_job), family="binomial", data = dftrain)

summary(mi1)
step(mi1, k= log(nrow(dftrain))) # poly(experience, 2):company_size 
Anova(mi1, test="LR")
anova(m12, mi1, test="Chisq") 
AIC(m10, m12, mi1)
```
```{r}
mi1 <- glm(target ~ training_hours*(enrolled_university + major_discipline + 
    company_size + last_new_job), family="binomial", data = dftrain)
...
AIC(m10, m12, mi1)
```
glm(target ~ poly(city_development_index, 3) + poly(experience, 2) + 
    training_hours + enrolled_university + major_discipline + 
    company_size + last_new_job
...
mi2 <- glm(target ~ relevent_experience*(enrolled_university + major_discipline + company_size + last_new_job), family="binomial", data = dftrain)

step(mi2, k= log(nrow(dftrain))) # relevent_experience:last_new_job

mi3 <- glm(target ~ enrolled_university*(relevent_experience + major_discipline + company_size + last_new_job), family="binomial", data = dftrain)

step(mi3, k= log(nrow(dftrain))) # nothing

mi4 <- glm(target ~ major_discipline*(enrolled_university + relevent_experience + company_size + last_new_job), family="binomial", data = dftrain)

step(mi4, k= log(nrow(dftrain))) # nothing

mi5 <- glm(target ~ company_size*(enrolled_university + relevent_experience + major_discipline + last_new_job), family="binomial", data = dftrain)

step(mi5, k= log(nrow(dftrain))) # nothing

mi6 <- glm(target ~ last_new_job*(enrolled_university + relevent_experience + major_discipline + company_size), family="binomial", data = dftrain)

step(mi6, k= log(nrow(dftrain))) # last_new_job:relevent_experience
```
poly(city_development_index, 3) + poly(experience, 2) + 
    training_hours + enrolled_university + major_discipline + 
    company_size + last_new_job
...
```{r}
mfin <- glm(target ~ poly(city_development_index, 3) + poly(experience, 2) + 
    training_hours + enrolled_university + major_discipline + 
    company_size + last_new_job + relevent_experience:last_new_job + poly(city_development_index,3):company_size + poly(experience, 2):company_size , family="binomial", data = dftrain)
...
Looking at the confusion matrix, the model correctly identifies most candidates who will stay (high true negatives), but it struggles to detect those who plan to leave. Specifically, True Negatives (9781) are much higher than True Positives (623), so the model is good at predicting "will not leave". Also, False Negatives (1726) are relatively large, many candidates who actually plan to leave are misclassified as stayers. False Positives (1577) are also notable, meaning some candidates are incorrectly predicted as leavers. The model performs well for the majority class but has limited ability to capture the minority class (leavers).

We obtain a F1 score of 0.57, which is lower than expected, but if we compare with the dummy model (0.39776), it has improved significantly. Also, to improve this score, we fitted other models to see if adding variables would improve the prediction power, but even with the complete model (with all variables and interactions), the maximum F1 score obtained was 0.605. Due to the low difference in the F1 score, we prioritize a simpler model even if F1 is lower.

```{r}
prob <-mfin$fit #gives a vector of n observation with probability of voting of each individual
target.est <- ifelse(prob<0.4,0,1) #apply threshold
